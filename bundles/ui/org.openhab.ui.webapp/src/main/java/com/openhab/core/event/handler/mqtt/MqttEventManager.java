package com.openhab.core.event.handler.mqtt;
...
public class MqttEventManager {
	...
	final static Logger logger = LoggerFactory.getLogger(MqttEventManager.class);
	
	public static MqttEventManager mqttEventManager	=	null;
	...
	public static MqttEventManager getEventManager(){
		synchronized (lock) {
		if(mqttEventManager==null){
			mqttEventManager	=	new MqttEventManager();
			mqttEventManager.init();
			if(logger.isInfoEnabled()){
				logger.info("Successfully inittialized MQTT Publisher");
			}
		}
		}
		return mqttEventManager;
